  ...
  //Debug.Log(gameType);
        if(gameType == GameType.InitialMenu) {
            ...
            if (currentEntity is Player) {
                ...
                if ((selectionType == SelectionType.SelectCardToPlay || selectionType == SelectionType.SelectCard)) {

                int numCards = currentPlayer.GetNumCards();
                                Debug.Log("num cards " + numCards);
                ...
                switch (key)
                {
                    ...
                    case KeyCode.Return:
                        if (menu.Select() == "Play Again?")
                        {
                            ...
                            for (int i = 0; i < menus.Length; i++) menus[i].DestroyAll();
                            Awake();
                            Start();
                        }
                        break;
